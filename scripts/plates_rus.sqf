// dshd76_plate setObjectTextureGlobal [0, "scripts\plates_rus\dshd76_plate_ca.paa"];
// obrcpn346_plate setObjectTextureGlobal [0, "scripts\plates_rus\obrcpn346_plate_ca.paa"];
// obrmp40_plate setObjectTextureGlobal [0, "scripts\plates_rus\obrmp40_plate_ca.paa"];
// wagner_sho2_plate setObjectTextureGlobal [0, "scripts\plates_rus\wagner_sho2_plate_ca.paa"];
// wagner_sho1_plate setObjectTextureGlobal [0, "scripts\plates_rus\wagner_sho1_plate_ca.paa"];
// wagner_sho3_plate setObjectTextureGlobal [0, "scripts\plates_rus\wagner_sho3_plate_ca.paa"];
// wagner_sho5_plate setObjectTextureGlobal [0, "scripts\plates_rus\wagner_sho5_plate_ca.paa"];